...
export default function RecuPage() {
    ...
    const [allStudents, setAllStudents] = useState<any[]>([]); // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    const loadReceipts = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                const mappedReceipts: Receipt[] = apiInscriptions.map((ins: any) => ({ // eslint-disable-line @typescript-eslint/no-explicit-any
                    id: ins.id,
                    date: ins.date.substring(0, 10),
                    time: new Date(ins.date).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }),
                    receiptNumber: ins.type === 'SOUTIEN' ? `EnovaSS ${String(ins.id).slice(-3)}` : `EnovaFP ${String(ins.id).slice(-3)}`,
                    issuedTo: `${ins.student?.name || ''} ${ins.student?.surname || ''}`.trim() || 'Inconnu',
                    studentId: ins.student?.id,
    ...
    const generateReceiptNumber = (type: 'Soutien' | 'Formation' | 'Other') => {
        let prefix = 'recu';
        if (type === 'Soutien') prefix = 'EnovaSS';
        else if (type === 'Formation') prefix = 'EnovaFP';
        ...
        // Format: Prefix + Space + 3 digits (e.g., EnovaSS 001)
    ...
    const filteredStudents = useMemo(() => {
        ...
        if (formData.receiptType === 'Soutien') {
            students = students.filter(s => s.inscriptions?.some((i: any) => i.type === 'SOUTIEN')); // eslint-disable-line @typescript-eslint/no-explicit-any
        } else if (formData.receiptType === 'Formation') {
            students = students.filter(s => s.inscriptions?.some((i: any) => i.type === 'FORMATION')); // eslint-disable-line @typescript-eslint/no-explicit-any
        }
            ...
            <div className="p-5">
                ...
                <div className="pt-4 border-t border-gray-200 text-sm space-y-2">
                    <p className="flex items-center gap-2 text-gray-700">
                        <User size={14} /> <strong>Client:</strong>
                        {receipt.studentId ? (
                            <a
                                href={`/admin/students?highlight=${receipt.studentId}`}
                            ...
                            <span className="ml-1">{receipt.issuedTo}</span>
                        )}
                    </p>
                    <p className="flex items-center gap-2 text-gray-700">
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        if (formData.receiptType === 'Formation') {
                                                            ...
                                                            if (formationsInscriptions && formationsInscriptions.length > 0) {
                                                                newItems = formationsInscriptions.map((i: any) => ({ // eslint-disable-line @typescript-eslint/no-explicit-any
                                                                    description: `Formation: ${i.category || 'N/A'}`,
                                                                    amount: i.amount || 0
                                                                }));
                                                            }
                                                        } else if (formData.receiptType === 'Soutien') {
                                                            const soutienInscriptions = student.inscriptions?.filter((i: any) => i.type === 'SOUTIEN'); // eslint-disable-line @typescript-eslint/no-explicit-any
                                                            if (soutienInscriptions && soutienInscriptions.length > 0) {
                                                                newItems = soutienInscriptions.map((i: any) => ({ // eslint-disable-line @typescript-eslint/no-explicit-any
