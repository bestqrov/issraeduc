...
export default function FormationDashboard() {
    ...
    const [stats, setStats] = useState<{
        ...
        recentInscriptions: any[] // eslint-disable-line @typescript-eslint/no-explicit-any
    }>({
        totalFormations: 0,
        totalInscriptions: 0,
        totalRevenue: 0,
    ...
    const handleCreateSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const errorMessage = error instanceof Error && 'response' in error
                ? (error as any).response?.data?.error || error.message // eslint-disable-line @typescript-eslint/no-explicit-any
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                            ...
                            <div className="space-y-4">
                                {!stats.recentInscriptions || stats.recentInscriptions.length === 0 ? (
                                    <p className="text-gray-500 text-center py-4">Aucune inscription récente</p>
                                ) : (
                                    stats.recentInscriptions.map((inscription: { id: string; student?: { name?: string; firstName?: string; lastName?: string }; createdAt: string; amount: number }) => (
                                        <div key={inscription.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                                            <div className="flex items-center gap-4">
                                                <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold">
                                                    {inscription.student?.firstName?.charAt(0) || 'U'}
                                                </div>
                                                <div>
                                                    <p className="font-semibold text-gray-800">{inscription.student?.firstName} {inscription.student?.lastName}</p>
