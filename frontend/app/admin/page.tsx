    ...
    const loadDashboardData = async (showLoading = true) => {
        try {
            ...
            try {
                ...
                // sort by createdAt desc and take 6 for quick access
                const recent = students.sort((a: any, b: any) => (new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())).slice(0, 6);
            ...
            try {
                ...
                const theoreticalRevenue = studentAnalytics.monthlyRevenue || 0; // What should be collected
                const actualInvoiced = studentAnalytics.actualMonthlyRevenue || 0; // What was invoiced
                const formationRevenue = formationAnalytics.monthlyRevenue || 0; // Formation revenue this month

                // Total theoretical revenue this month
                const totalTheoreticalRevenue = theoreticalRevenue + formationRevenue;

                // Cash Flow - actual money received
                ...
                // Unpaid = Theoretical - Actually Received
                const unpaid = Math.max(0, totalTheoreticalRevenue - totalReceived);
                ...
                setStats(prev => ({
                    ...
                    studentsThisMonth,
                    totalEmployees,
                    revenueThisMonth: totalTheoreticalRevenue, // Theoretical revenue for this month
                    totalReceivedMonth: totalReceived, // Actual cash received
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <button onClick={() => router.push('/admin/inscriptions')} className="p-4 rounded-xl bg-gray-50 hover:bg-green-50 hover:text-green-600 transition-colors text-center group">
                                <Plus className="mx-auto mb-2 text-gray-400 group-hover:text-green-500" size={24} />
                                <span className="text-sm font-medium">Nouvelle Étudiant</span>
                            </button>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        <div className="p-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                            <h4 className="font-semibold text-gray-900 text-sm">Derniers Élèves</h4>
                            <Link href="/admin/students" className="text-xs text-blue-600">Voir tous</Link>
                        </div>
                        <div className="p-4 space-y-2">
                            {recentStudents.length === 0 ? (
                                <p className="text-xs text-gray-500">Aucun élève récent</p>
                            ) : (
                                recentStudents.map(s => (
                                    <div key={s.id} className="flex items-center justify-between">
                                        <Link href={`/admin/students?highlight=${s.id}`} className="text-sm text-gray-800 hover:text-blue-600">{s.name} {s.surname}</Link>
