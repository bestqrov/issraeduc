...
export default function SettingsPage() {
    ...
    // Load settings from backend (if admin)
    useEffect(() => {
        const load = async () => {
            try {
                const res = await api.get('/settings');
                const settings = res.data.data;
                const contact = settings.contactInfo ? JSON.parse(settings.contactInfo) : {};
                ...
                setSchoolProfile(prev => ({
                    ...
                    director: contact.director || prev.director,
                }));
            } catch (e) {
                // ignore if not allowed or not found
                console.debug('Could not load settings from API', e);
            }
        };
    ...
    const fetchUsers = async () => {
        try {
            ...
            if (err.response?.status === 401 || err.response?.data?.message?.includes('Authentication')) {
                ...
                setTimeout(() => {
                    window.location.href = 'https://appenova.techmar.cloud';
                }, 2000);
            } else {
    ...
    const handleSaveSchoolProfile = async () => {
        ...
        try {
            // Persist to backend settings (ADMIN only)
            const contactInfo = {
            ...
            // Also save locally for instant UX
