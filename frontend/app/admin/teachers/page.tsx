...
const TeachersContent = () => {
    ...
    const copyTeacherLink = (link: string | undefined, id: string) => {
        if (!link) return;
        const fullLink = `${window.location.origin}/teacher/view/${link}`;
                    ...
                    <div key={teacher.id} className="group bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-lg transition-all duration-300">
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button
                                    onClick={() => copyTeacherLink((teacher as any).teacherLink, teacher.id)}
                                    className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg flex items-center gap-1 text-[10px] font-bold"
                                    title="Copier le lien d'accès"
                                >
                                    {copiedId === teacher.id ? <Check size={14} /> : <LinkIcon size={16} />}
                                    {copiedId === teacher.id ? 'Copié' : ''}
                                </button>
