...
export default function SoutienInscriptionForm({ onSuccess, onSuccessRedirect }: SoutienInscriptionFormProps) {
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    pricingData.forEach((item: any) => {
                        ...
                        const normLevel = levelRaw.toUpperCase()
                            .normalize("NFD")
                            .replace(/[\u0300-\u036f]/g, "");

                        if (normLevel.includes('LYCEE')) level = 'LYCEE';
                        else if (normLevel.includes('COLLEGE')) level = 'COLLEGE';
                        else if (normLevel.includes('PRIMAIRE')) level = 'PRIMAIRE';
                        else level = normLevel; // Fallback to normalized version
                    ...
                    console.log("Processed Subjects Mapping:", newSubjects);
        ...
        printWindow.document.write(`
            ...
            <body>
                ...
                <div class="medium">
                    ...
                    <div>ID Étudiant: ${lastRegisteredStudent.id || 'N/A'}</div>
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const studentData = response.data.data; // Get the created student data

            setLastRegisteredStudent({
                id: studentData.id, // Add student ID
                studentLink: studentData.studentLink, // Add student link
                name: formData.name,
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4 shadow-2xl">
                            <div className="text-center mb-6">
                                ...
                                <p className="text-gray-600 mb-4">L&apos;inscription a été enregistrée avec succès.</p>
                                {lastRegisteredStudent?.id && (
                                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                                        <p className="text-sm text-blue-800 font-medium mb-2">ID Étudiant:</p>
                                        <p className="text-lg font-bold text-blue-900 font-mono">{lastRegisteredStudent.id}</p>
                                        <p className="text-xs text-blue-600 mt-2">
                                            Utilisez cet ID pour accéder au tableau de bord de l&apos;étudiant
                                        </p>
                                    </div>
                                )}
                                {lastRegisteredStudent?.studentLink && (
                                    <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                                        <p className="text-sm text-green-800 font-medium mb-2">Lien d'Accès Étudiant:</p>
                                        <p className="text-lg font-bold text-green-900 font-mono break-all">{lastRegisteredStudent.studentLink}</p>
                                        <p className="text-xs text-green-600 mt-2">
                                            Ce lien unique permet à l&apos;étudiant d&apos;accéder à son tableau de bord
                                        </p>
                                        <button
                                            onClick={() => navigator.clipboard.writeText(`${window.location.origin}/student/${lastRegisteredStudent.studentLink}`)}
                            ...
                            {lastRegisteredStudent?.id && (
                                <Button
                                    onClick={() => window.open(`/student/${lastRegisteredStudent.id}`, '_blank')}
                                    className="w-full bg-purple-600 hover:bg-purple-700 text-white flex items-center justify-center gap-2"
                                >
                                    <User size={18} /> Voir le tableau de bord étudiant
                                </Button>
                            )}
                        </div>
                    </div>
