...
export async function getStudentById(id: string): Promise<Student> {
    // Try fetching by public link first (slug), then by numeric/uuid id
    try {
        const byLink = await api.get<ApiResponse<Student>>(`/students/link/${id}`);
        return byLink.data.data;
    } catch (err) {
...
export async function setStudentLink(id: string, slug?: string) {
    const response = await api.post<ApiResponse<any>>(`/students/${id}/link`, { slug });
    return response.data.data;
}
