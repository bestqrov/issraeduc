...
import { generateStudentLink } from '../students/students.service';
...
export const updateInscription = async (
    ...
    // Generate student link if not exists
    if (!inscription.student.studentLink) {
        try {
            const year = inscription.date.getFullYear();
            const studentLink = await generateStudentLink(inscription.student.name, inscription.student.surname, year);
            await prisma.student.update({
                where: { id: inscription.studentId },
                data: { studentLink },
            });
            // Update the returned inscription's student
            inscription.student.studentLink = studentLink;
        } catch (error) {
            console.error('Failed to generate student link:', error);
            // Don't throw, as inscription is successful
        }
    }
