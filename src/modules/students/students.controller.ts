...
export const create = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        console.log('Creating student with data:', { name, surname, phone, email, birthDate, schoolLevel });
        ...
        if (birthDate) {
            try {
                ...
                sendError(res, 'Invalid birth date format', 'Validation error', 400);
                return;
            }
        }
        ...
        sendError(res, error.message || 'Failed to create student', 'Database error', 500);
    }
};
...
export const getByLink = async (req: Request, res: Response): Promise<void> => {
    try {
        const { link } = req.params;
        const student = await getStudentByLink(link);
        sendSuccess(res, student, 'Student retrieved successfully', 200);
    } catch (error: any) {
        sendError(res, error.message, 'Failed to retrieve student', 404);
    }
...
export const setLink = async (req: Request, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        const { slug } = req.body;
        const student = await setStudentLink(id, slug);
        sendSuccess(res, student, 'Student link set successfully', 200);
    } catch (error: any) {
        sendError(res, error.message, 'Failed to set student link', 400);
    }
};
