...
export const getTeacherByLink = async (req: Request, res: Response) => {
    try {
        const teacher = await teachersService.getTeacherByLink(req.params.link);
        sendSuccess(res, teacher, 'Teacher retrieved successfully');
    } catch (error: any) {
        sendError(res, error.message, 'Failed to retrieve teacher', 404);
    }
};
